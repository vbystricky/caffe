...
namespace caffe {
    ...
    public:
        ...
        static DiagonalAffineMap identity(const int nd) {
            return DiagonalAffineMap(vector<pair<Dtype, Dtype> >((const size_t)nd, pair<Dtype, Dtype>((Dtype)1, (Dtype)0)));
        }
    ...
    DiagonalAffineMap<Dtype> FilterMap(const int kernel_h, const int kernel_w,
        ...
        coefs.push_back(make_pair(static_cast<Dtype>(stride_h),
            static_cast<Dtype>(kernel_h - 1) / 2 - pad_h));
        coefs.push_back(make_pair(static_cast<Dtype>(stride_w),
